...
async function popupMain() {
        ...
        if (isMobile) {
            ...
            if (spawn && config.invisibleUINotify) {
                ...
                browser.notifications
                    .create("pageSpawnedAway", {
                        title: "Hoardy-Web: REMINDER",
                        message: escapeNotification(
                            config,
                            "The newly spawned page might be hidden. See the list of open tabs or the list open private tabs, depending on the browser's mode." +
                                annoyingNotification(
                                    config,
                                    "Generate notifications about > ... mobile UI quirks",
                                ),
                        ),
                ...
                // Firefox on Android does not switch to new tabs opened from the settings
                browser.notifications
                    .create("pageNotSpawnedAway", {
                        title: "Hoardy-Web: REMINDER",
                        message: escapeNotification(
                            config,
                            `To make the search work on the "Help" page, enable "User Interface and Accessibility > Spawn internal pages in new tabs" option, open the "Help" page again, and then switch to the newly spawned tab.` +
                                annoyingNotification(
                                    config,
                                    "Generate notifications about > ... mobile UI quirks",
                                ),
    ...
    versionButton.onclick = catchAll(() =>
        resetAndReplace(false, { seenChangelog: true }, showChangelog, ""),
    );
    ...
    buttonToAction("showWindowState", () =>
        replaceWith(false, showState, narrowSessionId, narrowWindowId, null, "top"),
    );
    buttonToAction("showTabState", () =>
        replaceWith(false, showState, narrowSessionId, null, narrowTabId, "top"),
    );
    buttonToAction("showSaved", () => replaceWith(false, showSaved, "top"));
    buttonToAction("showBuggedOut", () =>
        replaceWith(false, showState, null, null, null, "buggedOut"),
    );
    ...
    buttonToMessage("rearchiveAllSaved", () => [
        "rearchiveSaved",
        {},
        true,
    ...
    buttonToMessage("rearchiveAdjunctSaved", () => [
        "rearchiveSaved",
        {},
        false,
        rearchive.andRewrite,
    ...
    function updateUI() {
        ...
        implySetConditionalClass(
            dbody,
            "on-archive-unsafe",
            "hidden",
            !(
                config.archive &&
                config.archiveExportAs &&
                !config.archiveSubmitHTTP &&
                !config.archiveSaveLS
            ),
        );
        ...
        implySetConditionalClass(
            dbody,
            "on-rearchive-attention",
            "hidden",
            !(config.rearchiveExportAs && rearchive.andDelete),
        );
        implySetConditionalClass(
            dbody,
            "on-rearchive-carefully",
            "hidden",
            !((config.rearchiveExportAs || config.rearchiveSubmitHTTP) && rearchive.andDelete),
        );
        implySetConditionalClass(
            dbody,
            "on-rearchive-unsafe",
        ...
        implySetConditionalOff(
            dbody,
            "on-exportAs",
            !((config.archive && config.archiveExportAs) || config.rearchiveExportAs),
        );
        ...
        implySetConditionalOff(
            ...
            "on-useHTTP",
            !(
                (config.archive && config.archiveSubmitHTTP) ||
        ...
        implySetConditionalOff(
            dbody,
            "on-LS",
            !(config.stash || (config.archive && config.archiveSaveLS)),
        );
        implySetConditionalOff(
            dbody,
            "on-auto",
        ...
        implySetConditionalOff(
            dbody,
            "on-autoUnmarkProblematicSimilar",
            !config.autoUnmarkProblematicSimilar,
        );
    ...
    async function updateStats(stats) {
        ...
        implySetConditionalClass(
            ...
            "on-reload",
            "hidden",
            stats.reload_pending || !(hash || stats.update_available || config.debugRuntime),
        );

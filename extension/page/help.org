...
Successfully re-archived reqres can be deleted automatically by setting {{{div2(rearchive.andDelete,=... and delete everything afterwards=)}}} and a similar option on the {{{button2(showSaved,=Saved in Local Storage= page)}}}.
However, these options should be used with caution:

- The behaviour of the {{{button2(rearchiveAdjunctSaved,=Re-archive adjunct= button)}}} with {{{div2(rearchive.andDelete,that option)}}} enabled can be surprising if you sometimes use both available {{{div2(rearchive-methods,re-archival methods)}}} and sometimes only one of them.
  See its help string for more info.

- Re-archiving by the means of {{{config2(rearchiveExportAs,=Export via 'saveAs'=)}}} with {{{div2(rearchive.andDelete,that option)}}} enabled is [[#unsafe-saveas][unsafe because of browser API limitations]].

  If you want to do this safely, you should:

  - disable {{{config2(archive,=Archive 'collected' reqres= option)}}};
  - re-archive with {{{div2(rearchive.andDelete,deletion)}}} disabled, by using any one of {{{div2(rearchive-actions,the buttons)}}};
  ...
  - press {{{button2(rearchiveAdjunctSaved,=Re-archive adjunct= button)}}}.

  The last action should then produce an =Re-archived and deleted <N> reqres invoking 0 archiving actions and <N> deletions from local storage.= notification.
  ...
  Keeping {{{config2(archive,=Archive 'collected' reqres= option)}}} enabled will not break anything in the internal machinery, but if another reqres gets captured and then sneakily =saved= a moment before you press the {{{button2(rearchiveAdjunctSaved,=Re-archive adjunct= button)}}} and the generated =WRR bundle= then [[#unsafe-saveas][silently fails to be saved to disk via =saveAs=]], then that sneaky reqres will still get deleted, and, thus, permanently lost.

Also, note that the {{{div2(rearchive.andDelete,deletion option)}}} and its equivalent on {{{button2(showSaved,=Saved in Local Storage= page)}}} are ephemeral and will be reset the next time you reopen the popup or that page.
...
Thus, you can simply use =Hoardy-Web= extension configured to {{{config2(archiveSaveLS,=Save reqres into local storage=)}}}, which is the default, and then {{{config2(rearchiveExportAs,export your dumps manually)}}} at the end of a browsing session via [[#re-archival][re-archival]], which would take a couple of clicks.
...
- simulate {{{button2(rearchiveAdjunctSaved,=Re-archive adjunct= button)}}} press with {{{div2(local.rearchiveDelete,deletion)}}} disabled, {{{shortcut(rearchiveAdjunctSaved)}}};

...
- The behaviour of the {{{button2(rearchiveAdjunctSaved,=Re-archive new= button)}}} with {{{div2(rearchive.andDelete,that option)}}} enabled can be surprising if you sometimes use both available {{{div2(rearchive-methods,re-archival methods)}}} and sometimes only one of them.
...
- Re-archiving by the means of {{{config2(rearchiveExportAs,=Export via 'saveAs'=)}}} with {{{div2(rearchive.andDelete,that option)}}} enabled is [[#unsafe-saveas][unsafe because of browser API limitations]].
  ...
  Keeping {{{config2(archive,=Archive 'collected' reqres= option)}}} enabled will not break anything in the internal machinery, but if another reqres gets captured and then sneakily =saved= a moment before you press the {{{button2(rearchiveAdjunctSaved,=Re-archive new= button)}}} and the generated =WRR bundle= then [[#unsafe-saveas][silently fails to be saved to disk via =saveAs=]], then that sneaky reqres will still get deleted, and, thus, permanently lost.

...
** Can I capture web pages while running my browser with JavaScript disabled?
Yes.
** Can I capture web pages that use a lot of JavaScript?
=pWebArc= will capture everything your browser fetches as you browse, except for, at the moment, WebSockets data.

In the most general case, "from scratch" replay of pages dynamically generated via JavaScript is not guaranteed.
E.g. consider a web page where its JavaScript generates a random number, then queries its server with that number, and renders the result.
Obviously, this can not be replayed "from scratch", since it will generate a new random number and the archive won't have a server's response for it.
Which is why [[./popup.html#snapshotTab][DOM snapshots]] exist, see the following question.
** Can I capture a web page as it currently is, after all JavaScript was run, not as it was when it was last fetched from the network?
:PROPERTIES:

...
When {{{config2(problematicNotify,this option)}}} is enabled, new notifications will be produced each time a new =problematic= reqres appears.
...
- does not prevent generation of new =canceled= reqres when a corresponding =Track new requests= toggle is also enabled, and they can be seen in the {{{button2(showState,history)}}}-{{{button2(showTabState,log)}}}.

In the latter case, those newly generated =canceled= reqres will also be marked as =problematic= if {{{config2(markProblematicWithImportantErrors,that)}}} option is enabled.
So, for convenience, there is also a {{{config2(workOfflineImpure,toggle)}}} that controls whether toggling =Work offline= options (from the popup or with [[#keyboard-shortcuts][keyboard shortcuts]]) should also automatically set the corresponding =Track new requests= option to the opposite value.
...
- toggle {{{tabconfig2(workOffline,=Work Offline= mode in the currently active tab)}}} and its children (the action depends on {{{config2(workOfflineImpure,impure toggling)}}}), {{{shortcut(toggleTabConfigWorkOffline)}}};
- toggle {{{tabconfig2(children.workOffline,=Work Offline= mode in the currently active tab's new children)}}} (the action depends on {{{config2(workOfflineImpure,impure toggling)}}}), {{{shortcut(toggleTabConfigChildrenWorkOffline)}}};

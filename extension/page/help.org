...
- if [[./popup.html#div-config.archiveExportAs][=Export dumps via 'saveAs'= option]] is enabled but [[./popup.html#div-config.exportAsBundle][bundling]] is disabled, =Hoardy-Web= will simply
  - compress the =dump=, [[./popup.html#div-config.gzipExportAs][if enabled]], and
  - export the resulting [[../../doc/data-on-disk.md][=WRR file=]] via browser's =saveAs= mechanism (i.e. generate a fake-Download);

- if both [[./popup.html#div-config.archiveExportAs][=Export dumps via 'saveAs'= option]] and [[./popup.html#div-config.exportAsBundle][bundling]] are enabled, =Hoardy-Web= will
  - append the =dump=, as a byte string, to a (per-=bucket=, see below) [[../../doc/data-on-disk.md][=WRR bundle=]],
  - and then, if the =WRR bundle= gets larger than [[./popup.html#div-config.exportAsMaxSize][this]] or after a delay controlled by [[./popup.html#div-config.exportAsInFlightTimeout][that]] and [[./popup.html#div-config.exportAsTimeout][this]] options,
    - compress the result, [[./popup.html#div-config.gzipExportAs][if enabled]], and
    - export the resulting =WRR bundle= via browser's =saveAs= mechanism (i.e. generate a fake-Download);

- if [[./popup.html#div-config.archiveSubmitHTTP][=Submit dumps via 'HTTP'= option]] is enabled, =Hoardy-Web= will submit the =dump= to the archiving server at [[./popup.html#div-config.submitHTTPURLBase][=Server URL= setting]] by making an =HTTP POST= request with the =dump= as request body (which is denoted by =srvIO= state on the diagram above);
...
- if [[./popup.html#div-config.archiveSaveLS][=Save reqres into local storage= option]] is enabled, =Hoardy-Web= will
...
- if [[./popup.html#div-config.archiveSaveLS][=Save reqres into local storage= option]] is disabled, =Hoardy-Web= will simply discard the reqres from memory, as if saving succeeded.

In the cases of stashing and archiving to local storage, the =dump='s get compressed if [[./popup.html#div-config.gzipLSDumps][this option]] is enabled.
...
- thus separating out those reqres into separate =saveAs= =WRR bundle='s with "share" in their name and/or putting them into a separate archiving server directory named "share", depending on [[./popup.html#archive-methods][archival methods]].
...
- You want to collect and accumulate lots of reqres before re-archiving them by the means of [[./popup.html#div-config.rearchiveExportAs][=Export via 'saveAs'=]] to make the UI less annoying and to [[./popup.html#div-config.gzipExportAs][compress]] the resulting =WRR bundle='s better.
...
Successfully re-archived reqres can be deleted automatically by setting [[./popup.html#div-rearchive.andDelete][=... and delete everything afterwards=]] and a similar option on the [[./popup.html#showSaved][=Saved in Local Storage= page]].
However, these options should be used with caution:

- The behaviour of the [[./popup.html#rearchiveAdjunctSaved][=Re-archive adjunct= button]] with [[./popup.html#div-rearchive.andDelete][that option]] enabled can be surprising if you sometimes use both available [[./popup.html#rearchive-methods][re-archival methods]] and sometimes only one of them.
  See its help string for more info.

- Re-archiving by the means of [[./popup.html#div-config.rearchiveExportAs][=Export via 'saveAs'=]] with [[./popup.html#div-rearchive.andDelete][that option]] enabled is [[#unsafe-saveas][unsafe because of browser API limitations]].

  If you want to do this safely, you should:

  - disable [[./popup.html#div-config.archive][=Archive 'collected' reqres= option]];
  - re-archive with [[./popup.html#div-rearchive.andDelete][deletion]] disabled, by using any one of [[./popup.html#rearchive-actions][the buttons]];
  - confirm that all generated fake-Downloads were actually successfully saved to disk;
  - enable [[./popup.html#div-rearchive.andDelete][deletion]] without changing any other options;
...
Also, note that the [[./popup.html#div-rearchive.andDelete][deletion option]] and its equivalent on [[./popup.html#showSaved][=Saved in Local Storage= page]] are ephemeral and will be reset the next time you reopen the popup or that page.
...
- [[./popup.html#replayAll][this button]] re-navigates each open tab for which [[./popup.html#div-tabconfig.replayable][this setting]] is enabled to its replay page;

- [[./popup.html#replayTabBack][this buttons]] re-navigates the currently active tab regardless of any settings.
...
- [[./popup.html#div-tabconfig.autoReplay][this toggle]] enables =Auto-Replay= mode in the currently active tab;

- [[./popup.html#div-tabconfig.children.autoReplay][this toggle]] enables it for the currently active tab's new children, thus making all link middle-clicks open replay pages;

- there is also a [[./popup.html#div-config.root.autoReplay][toggle]] for controlling the default value of the above two options in newly spawned root tabs.

=Auto-Replay= attempts will be silently skipped if the archiving server does not support it or when [[./popup.html#div-config.replaySubmitHTTP][this option]] is disabled.

Currently =Auto-Replay= mode only works for top-level navigation requests and performs redirects regardless of your [[./popup.html#div-config.submitHTTPURLBase][archiving server]] having an archive of the URL in question.
...
Thus, you can simply use =Hoardy-Web= extension configured to [[./popup.html#div-config.archiveSaveLS][=Save reqres into local storage=]], which is the default, and then [[./popup.html#div-config.rearchiveExportAs][export your dumps manually]] at the end of a browsing session via [[#re-archival][re-archival]], which would take a couple of clicks.

Or, alternatively, you can switch to using [[./popup.html#div-config.archiveExportAs][=Export dumps via 'saveAs'=]] by default instead, disabling the other archiving methods.
...
- toggle [[./popup.html#div-tabconfig.autoReplay][=Auto-Replay= mode in the currently active tab]] and its children, {{{shortcut(toggleTabConfigAutoReplay)}}};
- toggle [[./popup.html#div-tabconfig.children.autoReplay][=Auto-Replay= mode in the currently active tab's new children]], {{{shortcut(toggleTabConfigChildrenAutoReplay)}}};
- toggle [[./popup.html#div-tabconfig.workOffline][=Work Offline= mode in the currently active tab]] and its children (the action depends on [[./popup.html#div-config.workOfflineImpure][impure toggling]]), {{{shortcut(toggleTabConfigWorkOffline)}}};
...
- toggle [[./popup.html#div-tabconfig.limbo][=Limbo= mode in the currently active tab]] and its children, {{{shortcut(toggleTabConfigLimbo)}}};
...
- toggle [[./popup.html#div-tabconfig.snapshottable][inclusion in global snapshots of the currently active tab]] and its children, {{{shortcut(toggleTabConfigSnapshottable)}}};
- toggle [[./popup.html#div-tabconfig.children.snapshottable][inclusion in global snapshots of the currently active tab's children]], {{{shortcut(toggleTabConfigChildrenSnapshottable)}}};
...
- toggle [[./popup.html#div-tabconfig.replayable][inclusion in global replays of the currently active tab]] and its children, {{{shortcut(toggleTabConfigReplayable)}}};
- toggle [[./popup.html#div-tabconfig.children.replayable][inclusion in global replays of the currently active tab's children]], {{{shortcut(toggleTabConfigChildrenReplayable)}}};
...
- open a link in a new tab or window with currently active tab's [[./popup.html#div-tabconfig.children.collecting][tracking in children tabs setting]] negated.
  I.e.,
...
- @@html:<span id="unsafe-saveas">@@ Browsers do not report a fake-Download file failing to be written to disk via =saveAs= API as an error.
  This makes [[./popup.html#div-config.archiveExportAs][=Export dumps via 'saveAs'= archival]] and [[./popup.html#div-config.rearchiveExportAs][similar re-archival]] options technically unsafe.
  @@html:</span>@@
...
Archival [[./popup.html#div-config.archiveExportAs][by exporting using =saveAs=]] (generation of fake-Downloads) can fail and **lose a bit of your collected data at a time** if you press a wrong button in you browser's UI, mis-reconfigure your browser a bit, or [[#unsafe-saveas][your disk gets out of space unexpectedly]].
...
Archival [[./popup.html#div-config.archiveSaveLS][to browser's local storage]] was added because it was very easy to implement after [[#stash][stashing]] was added.
...
As it is now, the only way to be absolutely sure you data is properly forever-saved to disk when the extension reports it archived is to use [[./popup.html#div-config.archiveSubmitHTTP][submission via =HTTP=]] or [[#re-archival][re-archive]] carefully.

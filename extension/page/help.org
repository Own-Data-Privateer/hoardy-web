...
This pages is written in =org-mode= markup language and its uses a lot of its advanced features, which GitHub does not implement, so this page is almost unreadable on GitHub.
...
Doing that will make this document interactive: the settings popup will be displayed on the right on this page and hovering over or clicking references to configuration options on this page will be highlighting them in the settings popup.
...
#+END_EXPORT
* Introduction
*This page documents the =Hoardy-Web= browser extension (add-on).*
(For =hoardy-web serve= replay server's documentation, see [[../../tool/][there]].
For documentation of other parts of this project, see [[../../][there]].)
Most notably, this page
...
- discusses some [[#workflows][additional workflows]] =Hoardy-Web= implements support for;
...
In =Hoardy-Web=, you can check the description of each status line, button, or configuration option by hovering over it or by pressing the nearby =?= button.
However, many of those descriptions might make little sense without you first learning the general operation principles of =Hoardy-Web=.
...
Press =?= symbols to see a tooltip explaining what each of them does.
...
- =snapshot=: this reqres was produced by taking a =DOM= (Document Object Model) snapshot, i.e., by capturing a raw =HTML= or =XML= of the current state of the tab/frame, not by capturing a network request;
  ...
  this can be done using {{{button2(snapshotAll,this)}}} or {{{button2(snapshotTab,that)}}} buttons, or similar [[#keyboard-shortcuts][keyboard shortcuts]];
...
*** Glossary
- An /in-flight reqres/ ({{{bothstats(in_flight)}}}) is a reqres that did not reach the =finished= state yet, in the log {{{logPage}}} such reqres will be shown to be in =in_flight= state.
...
- having a =partial request body= ([[#firefox-bug-no-post][Firefox]], [[#chromium-bug-no-post][Chromium]]), which is denoted by =partial= flag on the log page {{{logPage}}};

- having an =incomplete response body=, denoted by =incomplete= flag on the log page {{{logPage}}}, which also forces the reqres to stop in =incomplete= or =incomplete_fc= final networking state;
...
- having =buggy request metadata= or =buggy response metadata= ([[#firefox-bug-race][Firefox]], [[#chromium-bug-no-wre][Chromium]]), which is denoted by =buggy= flag on the log page {{{logPage}}}.

These flags influence some of the computations described below.

- Disabling {{{config2(archivePartialRequest,this toggle)}}} will disable archiving of reqres with =partial request body= flag set.
- Disabling {{{config2(archiveBuggy,this toggle)}}} will disable archiving of reqres with one of the =buggy * metadata= flags set.

Disabling these toggles is not recommended, however, as these flags are usually caused by browser bugs and are thus unfixable, unless you run =Hoardy-Web= on a patched browser.
...
- Disabling {{{config2(archiveIncompleteResponse,this toggle)}}} will disable archiving of reqres with =incomplete response body= flag set.

In contrast, this toggle is disabled by default because response-incomplete reqres are usually useless and can be easily fixed by reloading a page in question.

See [[../../doc/data-on-disk.md][=WRR= file format documentation]] on how these flags get actually recorded in the generated dumps.
*** Problematic reqres
:PROPERTIES:
...
- the log page {{{logPage}}}, which shows =problematic= reqres in a separate section;
- and nothing else.

*In other words, the =problematic= flag is purely a UI indicator, it does not influence archival or any of the other steps described below in any way.*
...
When {{{config2(problematicNotify,this option)}}} is not disabled, new notifications will be produced each time a new =problematic= reqres appears.
The latter behaviour can be disabled per-source using relevant options {{{perSource(problematicNotify)}}}.
...
The raw error strings reported by the browser for each reqres can be seen in the log {{{logPage}}}.
...
For example, say, you loaded a tab, some of its images failed to load completely and got marked as =problematic=, so you re-load that tab again, and then all reqres that failed to be fetched the first time get fetched properly the second time.

In theory, in such a situation, to be absolutely pedantic, you would then need to check that each of your =problematic= reqres actually got re-fetched and unmark those that did, in case the browser decided to skip fetching of some of them, which could happen with modern web lazy-loading features.
Obviously, actually doing this manually each time would be incredibly annoying.

Which is why =Hoardy-Web= provides a {{{div2(unproblematic-options,bunch of configuration options)}}} that automate unmarking of =problematic= reqres in various situations.

Additionally, when {{{config2(problematicNotify,this option)}}} is set to its right-most state, new notifications will be produced each time an old =problematic= reqres gets automatically unmarked, unless a relevant per-source option {{{perSource(problematicNotify)}}} is unset.
**** I don't care about any of this and notifications about =problematic= reqres annoy me!
:PROPERTIES:
:CUSTOM_ID: suppress-problematic
:END:

If you don't care about the =problematic= flag in a select tab and those notifications annoy you, you should disable {{{tabconfig2(problematicNotify,this option)}}}.
If they annoy you in general, you can disable the {{{config2(problematicNotify,global one)}}} instead.

You should probably not, however, disable too many of the options under {{{div2(problematic-options,=Mark reqres as 'problematic' when they finish= settings)}}}.
...
Each new =finished= reqres advances either to the =picked= or the =dropped= states, which influences the actions =Hoardy-Web= performs in the next steps.

Which of those two states gets selected is decided based on the {{{div2(pick-options,=Pick reqres for archival when they finish= options)}}}.
...
*** Glossary
- A /problematic reqres/ ({{{bothstats(problematic)}}}) is a /finished reqres/ that satisfies the conditions set by {{{div2(problematic-options,=Mark reqres as 'problematic' when they finish= settings)}}}.
...
- a =loggable=, which is a hollow =reqres= structure without any request or response data, i.e. it only keeps the metadata used by the log {{{logPage}}}, and
...
When {{{config2(archive,=Archive 'collected' reqres= toggle)}}} is enabled, those =queued= reqres proceed directly to the next step.
*** Limbo mode
:PROPERTIES:
:CUSTOM_ID: limbo
:END:
...
(These latter options mainly exists for debugging.)

If {{{config2(limboNotify,this option)}}} is enabled and there are more than {{{config2(limboMaxNumber,this number)}}} reqres =in_limbo= or the total size of all dumps =in_limbo= is more than {{{config2(limboMaxSize,this size)}}} (in MiB), =Hoardy-Web= will complain to remind you to /collect/ or /discard/ some of them so that your browser does not waste too much memory (and so that you won't loose too much data if something crashes while {{{config2(stash,=Stash 'collected' reqres into local storage= option)}}} discussed below is disabled).

Additionally, note that the default values of {{{div2(unproblematic-options,automatic unmarking of =problematic= reqres)}}} produce slightly imprecise results when combined with picking into limbo {{{perSource(limbo)}}}.
Thus, if you are feeling pedantic, so you might want to disable {{{config(autoUnmarkProblematicSimilarAcrossLimbo,that option)}}}.
Though, if you do that, the resulting behaviour =Hoardy-Web= will probably feel rather counter-intuitive, at least at first.
*** Glossary
...
You can also ask =Hoardy-Web= to forget all history manually by pressing {{{button2(forgetAllLog,this button)}}}, or to forget history of reqres generated by the currently active tab by pressing {{{button2(forgetAllTabLog,that button)}}} instead, or do the same by using similar buttons on the log page {{{logPage}}}.
Using the latter also allows the use of reqres filtering options available there for doing this, allowing you to selectively forget parts of history.
...
To forget about them, you will have to first unset the =problematic= flag on the respective reqres via {{{button2(unmarkAllProblematic,this button)}}}, or {{{button2(unmarkAllTabProblematic,that button)}}}, or use similar buttons on the log page {{{logPage}}}.
** Step 4: Archival
...
*** Glossary
- A /failed to archive reqres/ ({{{stat(unarchived)}}}) is a reqres that is currently =unarchived=, i.e. a reqres that failed to be archived by one of the enabled {{{div2(archive-methods,archival methods)}}}.
  It is also a part of the sum of the "Failed" part of the {{{stat2(queued_failed,Queued/Failed)}}} line.
...
- Unlike archival, re-archival is not automatic.
  ...
  You will have to manually press its {{{div2(rearchive-actions,popup UI action buttons)}}}, a similar button on the {{{button2(showSaved,=Saved in Local Storage= page)}}}, or [[#keyboard-shortcuts][keyboard shortcuts]].
...
- You run =Hoardy-Web= under a Fenix-based mobile browser, where {{{config2(archiveExportAs,Export via =saveAs=)}}} does not work, you are unable to run an {{{config2(submitHTTPURLBase,archiving server)}}} on the same device, because it's an un-rooted mobile phone, and you don't want to submit your dumps over the Internet, because you are feeling paranoid.
...
*** Deletion of successfully externally archived reqres
In use cases
...
However, the former two methods should be used with caution:
...
- Re-archiving by the means of {{{div2(rearchive-actions,re-archival action buttons)}}} (or their {{{button2(showSaved,=Saved in Local Storage= page)}}} equivalent) when both {{{config2(rearchiveExportAs,=Export via 'saveAs'=)}}} and {{{div2(rearchive.andDelete,deletion)}}} (or its {{{button2(showSaved,=Saved in Local Storage= page)}}} equivalent) are enabled is [[#unsafe-saveas][unsafe because of browser API limitations]].
  ...
  The essence of issue is that if a new reqres gets captured and then saved into local storage via the normal {{{config2(archive,=Archive 'collected' reqres=)}}} machinery right at the moment you press the {{{button2(rearchiveAdjunctSaved,=Re-archive new= button)}}} and the generated =WRR bundle= then [[#unsafe-saveas][silently fails to be saved to disk via =saveAs=]], then that sneaky reqres will still get deleted, and, thus, permanently lost.
  ...
  You can press those buttons one after the other immediately, the latter action will get queued for later execution until the former finishes, so they won't interfere with each other.
  ...
  Or, alternatively, as a last step, you can open {{{button2(showSaved,=Saved in Local Storage= page)}}}, set =Exported via 'saveAs'= filter to =true= and then press =Delete=.
...
However, note that re-archival by {{{config2(rearchiveSubmitHTTP,sumbission via =HTTP=)}}} with {{{div2(rearchive.andDelete,deletion)}}} enabled is safe to use.
...
Thus, since {{{config2(rearchiveSubmitHTTP,sumbission via =HTTP=)}}} does report its errors properly, successfully re-archived reqres can be immediately and safely deleted from local storage in that case, which would save you a lot of time and disk writes if you are re-archiving a lot of stuff all at once.
** (Optional) Step 5: Replay integration
:PROPERTIES:
:CUSTOM_ID: replay
:END:
...
=Auto-Replay= attempts will be silently skipped if the archiving server does not support it or when {{{config2(replaySubmitHTTP,this option)}}} is disabled.
...
There are also keyboard shortcuts and context menu actions for most of the above [[#shortcuts][described below]].
* Workflows
:PROPERTIES:
:CUSTOM_ID: workflows
:END:
...
Meanwhile, normally, when using the [[../../tool/][=hoardy-web= tool]], =HTTP= replay pages generated by its =serve= sub-command and static website mirrors generated by its =mirror= sub-command remap all URLs of page requisites to point to local replay URLs and files.
...
Say that, after being informed by the previous two paragraphs, you now want to ensure that

- your browser won't notify a page's origin server when you start interacting with it long after you loaded it, and
- your browser won't try to access the Internet when you open one of the =hoardy-web serve='ed or =hoardy-web mirror='ed pages.
...
- does not prevent generation/capture of new =canceled= reqres when a corresponding =Track new requests= toggles {{{config2(collecting,(glob))}}}, {{{perSource(collecting)}}} are enabled and {{{config2(collectingWorkOffline,this toggle)}}} is set;

  this makes it possible to inspect the resulting =canceled= reqres in the log {{{logPage}}};
...
Finally, there is also a {{{div2(special-tab-options,bunch options)}}} that automatically enable "Work offline" mode in tabs with various classes of URLs.
...
When running =Hoardy-Web= under Tor Browser, you probably want to configure it all in such a way so that all of the machinery of =Hoardy-Web= is completely invisible to web pages running under your Tor Browser, to prevent fingerprinting.
...
Using either of the above methods won't allow you to replay archived captured, though.
*** Replay
...
- open the {{{button2(showState,=Internal State and Logs= page)}}}, {{{shortcut(showState)}}};

- open the =Internal State and Logs= page, scrolled to the end of the log, {{{shortcut(showLog)}}};
- open the {{{button2(showTabState,=Internal State and Logs= page)}}} narrowed to the currently active tab's data, {{{shortcut(showTabState)}}};
...
- simulate {{{button2(rearchiveAdjunctSaved,=Re-archive new= button)}}} press with {{{div2(local.rearchiveDelete,deletion)}}} disabled, {{{shortcut(rearchiveAdjunctSaved)}}};
- toggle {{{tabconfig2(autoReplay,=Auto-Replay= mode in the currently active tab)}}} and its children, {{{shortcut(toggleTabConfigAutoReplay)}}};
- toggle {{{tabconfig2(children.autoReplay,=Auto-Replay= mode in the currently active tab's new children)}}}, {{{shortcut(toggleTabConfigChildrenAutoReplay)}}};
- toggle {{{tabconfig2(workOffline,=Work Offline= mode in the currently active tab)}}} and its children, {{{shortcut(toggleTabConfigWorkOffline)}}};
- toggle {{{tabconfig2(children.workOffline,=Work Offline= mode in the currently active tab's new children)}}}, {{{shortcut(toggleTabConfigChildrenWorkOffline)}}};
- toggle {{{tabconfig2(collecting,tracking of newly spawned =HTTP= requests in the currently active tab)}}}, {{{shortcut(toggleTabConfigTracking)}}};
- toggle {{{tabconfig2(children.collecting,tracking of newly spawned =HTTP= requests currently active tab's children)}}}, {{{shortcut(toggleTabConfigChildrenTracking)}}};
- {{{button2(unmarkAllProblematic,unmark all problematic reqres)}}}, {{{shortcut(unmarkAllProblematic)}}};
- {{{button2(unmarkAllTabProblematic,unmark all current tab's problematic reqres)}}}, {{{shortcut(unmarkAllTabProblematic)}}};
- toggle {{{tabconfig2(limbo,=Limbo= mode in the currently active tab)}}} and its children, {{{shortcut(toggleTabConfigLimbo)}}};
- toggle {{{tabconfig2(children.limbo,=Limbo= mode in currently active tab's children)}}}, {{{shortcut(toggleTabConfigChildrenLimbo)}}};
- {{{button2(collectAllInLimbo,collect all reqres from limbo)}}}, {{{shortcut(collectAllInLimbo)}}};
- {{{button2(collectAllTabInLimbo,collect all reqres from limbo for the currently active tab)}}}, {{{shortcut(collectAllTabInLimbo)}}};
- {{{button2(discardAllInLimbo,discard all reqres from limbo)}}}, {{{shortcut(discardAllInLimbo)}}};
- {{{button2(discardAllTabInLimbo,discard all reqres from limbo for the currently active tab)}}}, {{{shortcut(discardAllTabInLimbo)}}};
- toggle {{{tabconfig2(snapshottable,inclusion in global snapshots of the currently active tab)}}} and its children, {{{shortcut(toggleTabConfigSnapshottable)}}};
- toggle {{{tabconfig2(children.snapshottable,inclusion in global snapshots of the currently active tab's children)}}}, {{{shortcut(toggleTabConfigChildrenSnapshottable)}}};
- {{{button2(snapshotAll,take =DOM= snapshot of all tabs)}}} for which {{{tabconfig2(snapshottable,=Include in global snapshots= setting)}}} is enabled, {{{shortcut(snapshotAll)}}};
- {{{button2(snapshotTab,take =DOM= snapshot of the currently active tab)}}}, {{{shortcut(snapshotTab)}}}.
- toggle {{{tabconfig2(replayable,inclusion in global replays of the currently active tab)}}} and its children, {{{shortcut(toggleTabConfigReplayable)}}};
- toggle {{{tabconfig2(children.replayable,inclusion in global replays of the currently active tab's children)}}}, {{{shortcut(toggleTabConfigChildrenReplayable)}}};
- {{{button2(replayAll,replay all tabs)}}} for which {{{tabconfig2(replayable,=Include in global replays= setting)}}} is enabled, {{{shortcut(replayAll)}}};
- {{{button2(replayTabBack,replay the currently active tab)}}}, {{{shortcut(replayTabBack)}}}.
...
:PROPERTIES:
:CUSTOM_ID: errors-notifications
:END:
...
- =Replay is forbidden by the "Replay from the archiving server" option.=
  ...
  Un-disable {{{config2(replaySubmitHTTP,this option)}}}.
...
- =Failed to open/create a database via `IndexedDB` API, all data persistence will be done via `storage.local` API instead. This is not ideal, but not particularly bad. However, the critical issue is that it appears Hoardy-Web previously used `IndexedDB` for archiving and/or stashing reqres.=
  ...
  - Enable {{{div2(rearchive.andRewrite,=... and force a re-write= option)}}},
  - press {{{button2(rearchiveAdjunctSaved,=Re-archive new= button)}}},
...
- =webRequest::capture::CANCELED::BY_WORK_OFFLINE= is produced when the reqres was canceled by one of "Work offline" options, i.e. as a result of one or more of {{{config2(workOffline,this)}}}-{{{tabconfig2(workOffline,this)}}}-{{{tabconfig2(children.workOffline,this)}}}-{{{config2(root.workOffline,this)}}}-{{{config2(background.workOffline,or)}}}-{{{config2(extension.workOffline,that)}}} options being set.
* Quirks and Bugs
:PROPERTIES:
:CUSTOM_ID: bugs
:END:
...
- @@html:<span id="chromium-bug-no-wre">@@ Chromium sometimes simply does not generate any =webRequest= API events for a request it tracks.
  ...
  In this case, =Hoardy-Web= will collect no request data and metadata, this marking such reqres with both =partial request body= and =buggy request metadata=.
...
- =tabs= permission is used for tracking per-tab state and stats, making =Hoardy-Web='s toolbar icon show per-tab state, {{{button2(snapshotAll,taking =DOM= snapshot of all tabs)}}}, buttons switching to a related tab in the log {{{logPage}}}, etc.
...
Archival {{{config2(archiveExportAs,by exporting using =saveAs=)}}} (generation of fake-Downloads) can fail and **lose a bit of your collected data at a time** if you press a wrong button in you browser's UI, mis-reconfigure your browser a bit, or [[#unsafe-saveas][your disk gets out of space unexpectedly]].
...
If you question is about =hoardy-web serve= replay server, see [[../../tool/][there]].
If you question is about other parts of this project, see [[../../][there]].
If the those pages do not mention your problem, {{{reportit()}}}.

...
To view/replay your collected data, see the accompanying [[https://github.com/Own-Data-Privateer/pwebarc/tree/master/tool][=wrrarms= tool]] (also [[https://oxij.org/software/pwebarc/tree/master/tool/][there]]).
** Glossary
...
Whether =pWebArc= will track a given request depends on the =Track new reqres= toggles in the settings popup, e.g:
...
- open a given link in a new tab with currently active tab's [[./popup.html#div-tabconfig.children.collecting][tracking in children tabs setting]] negated.
  ...
  - selecting =pWebArc > Open Link in New Tracked/Untracked Tab= menu item,
...
** Relevant issues of Firefox-based mobile browsers: Firefox Android aka Fenix, Fennec, Mull, etc
:PROPERTIES:
...
- @@html:<span id="chromium-bug-detach">@@ You will get an annoying notification bar constantly displayed in the browser while [[./popup.html#div-config.collecting][this option]] is enabled.
...
- @@html:<span id="error-race">@@ =webRequest::pWebArc::RESPONSE::BROKEN= is produced when some response metadata is unavailable.
  @@html:</span>@@
...
** Can I use =pWebArc= to capture web pages while my browser runs with JavaScript disabled?
Yes.
** Can I use =pWebArc= to capture web pages that use a lot of JavaScript?
This is why [[./popup.html#snapshotTab][DOM]]-[[./popup.html#snapshotAll][snapshot]] buttons exist, see the following question.

In principle, =pWebArc= will capture everything your browser fetches from the network as you browse the web, except for, at the moment, WebSockets data.
So, web pages using only simple UI-related JavaScript code will work fine when you start replaying them "from scratch" via [[https://github.com/Own-Data-Privateer/pwebarc/tree/master/tool][=wrrarms export mirror=]] (also [[https://oxij.org/software/pwebarc/tree/master/tool/][there]]) or some such.
...
** Can I use =pWebArc= to capture a web page as it currently is, after all JavaScript was run, not as it was when it was last fetched from the network?
:PROPERTIES:
...
** How can I make =pWebArc= capture a web page completely, especially when parts of it are loaded lazily?
:PROPERTIES:
:CUSTOM_ID: faq-lazy
:END:
...
** Can I use =pWebArc= to capture a web page without archiving it, look at it, decide if I want to save it, and archive it only if I do, all without reloading the page a second time?
:PROPERTIES:
:CUSTOM_ID: faq-limbo
:END:
...
** Why do pages under [[https://addons.mozilla.org/]] and [[https://chromewebstore.google.com/]] can not be captured by =pWebArc=?
...
** When running =pWebArc= under Chromium, a lot of my captures fail with =debugger::pWebArc::EMIT_FORCED_BY_DETACHED_DEBUGGER=, =debugger::pWebArc::NO_RESPONSE_BODY::DETACHED_DEBUGGER=, =webRequest::pWebArc::NO_DEBUGGER::CANCELED=, and similar errors. What do I do?
:PROPERTIES:
...
** When running =pWebArc= under Firefox, some of my captures fail with =webRequest::pWebArc::RESPONSE::BROKEN=. What do I do?
:PROPERTIES:
...
** Why does a (specific) URL or some part of it fails to be properly captured by =pWebArc=?
...
** The documentation claims that all =pWebArc= archival methods except for [[./popup.html#div-config.archiveSubmitHTTP][submission via HTTP]] are unsafe. Why?
:PROPERTIES:
...
** When running =pWebArc= under Firefox, enabling [[./popup.html#div-config.archiveExportAs][export via =saveAs=]] makes the browser's UI quite annoying. Can it be fixed?
:PROPERTIES:

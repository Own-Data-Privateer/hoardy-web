...
- [[./popup.html#div-tabconfig.children.collecting][this one]] controls whether it will track new reqres originating from new tabs opened from the currently active tab (e.g. via middle mouse click, context menu, etc),
...
- You will get an annoying notification bar constantly displayed in the browser while [[./popup.html#div-config.collecting][pWebArc is enabled]], trying to close it will detach the debugger, so pWebArc will reattach immediately (because it assumes you don't want to lose data), and the bar will appear again.

  *Also, closing the notification will make all in-flight requests lose their response data.*
...
- On Chromium-based browsers, "New Tab" browser action (i.e. the plus sign in the tab bar, =Control+T=, menu item, etc) does not create a new root tab, instead it creates a new child tab of the currently active tab.
  In other words, on Chromium-based browser "New Tab" browser action uses [[./popup.html#div-tabconfig.children.collecting][this]] and [[./popup.html#div-tabconfig.children.profile][this]] settings instead of [[./popup.html#div-config.root.collecting][this]] and [[./popup.html#div-config.root.profile][this]] one.
  The latter pair only ever get used for a first tab in a new window (e.g. =Control+N=) and when the extension is (re-)loaded.
*** Known browser bugs

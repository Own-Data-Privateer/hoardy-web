...
- =incomplete_fc=: only on Firefox-based browsers: the browser loaded the response data of this reqres directly from its cache, but did not give it to pWebArc;
  ...
  this is just how Firefox handles things sometimes (usually, for images);
...
Moreover, the following section will discuss how pWebArc will also stash most =failed= to archive reqres into browser's local storage.
...
before you =collected= or =discarded= everything from =in_limbo= or pWebArc has successfully archived everything from its archived queue.
...
- which implies that, technically, stashing is not a silver bullet against data loss (to make it such would mean unconditional immediate stashing of all captured data, which would waste a lot of disk IO on most pWebArc configurations);
- stashing of =in_limbo= reqres [[./popup.html#div-tabconfig.stashLimbo][via]]-[[./popup.html#div-tabconfig.children.stashLimbo][one]]-[[./popup.html#div-config.root.stashLimbo][of]]-[[./popup.html#div-config.background.stashLimbo][the]]-[[./popup.html#div-config.extension.stashLimbo][those]] options is not free, keeping these options enabled if you have [[./popup.html#div-tabconfig.limbo]["Pick into limbo" option]] enabled yet almost never archive from limbo is a waste of disk IO, so you might want to disable some of those options in that case,

When both [[./popup.html#div-config.archive]["Archive =collected= reqres by" option]] and [[./popup.html#div-config.stash]["Stash =collected= reqres into local storage" option]] are disabled, then, when new reqres get =queued=, pWebArc will generate new desktop notifications complaining about them, unless [[./popup.html#div-config.archiveStuckNotify][that option]] is disabled too.

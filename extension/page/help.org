...
- =webNavigation= permission is used to apply {{{div2(special-tab-options,=Tabs with special URLs= options)}}} when a tab navigates to a different URL, see [[#work-offline][above]] for more info.

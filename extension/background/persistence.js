...
function recordOneAssumedBroken(accumulator, storeID, reason, archivable) {
    ...
    pushToIssueAcc(
        [accumulator[0], byReasonMap, accumulator[2]],
        reason,
        false,
        recoverable,
        archivable,
...
function scheduleRetryAllUnarchived(timeout) {
    if (
        config.archive &&
        reqresUnarchivedIssueAcc[0].size > 0 &&
        // and at least one error is recoverable
        Array.from(reqresUnarchivedIssueAcc[1].values()).some((byReasonMap) =>
            Array.from(byReasonMap.values()).some((unarchived) => unarchived.recoverable),
        )
    ) {
...
async function exportAsOne(archivable, bundleBuckets, unarchivedAccumulator) {
    ...
    let u = cacheSingleton(bundleBuckets, bucket, () => {
        return {
            queue: [],
            dumps: [],
            size: 0,
...
async function submitHTTPOne(archivable, unarchivedAccumulator) {
    ...
    if (!serverConfig.alive) {
        ...
        broken(
            config.submitHTTPURLBase,
            "this archiving server does not support archiving",
            false,
            true,
        );
        return false;
    ...
    if (
        config.submitHTTPParanoid &&
        recordOneAssumedBroken(
            unarchivedAccumulator,
            storeID,
    ...
    try {
        ...
        broken(
            config.submitHTTPURLBase,
            `\`Hoardy-Web\` can't establish a connection to the archiving server: ${errorMessageOf(err)}`,
            true,
        );
    ...
    if (response.status !== 200) {
        broken(
            storeID,
            `request to the archiving server failed with ${response.status} ${response.statusText}: ${responseText}`,
            false,
        );
        return false;
    }
...
async function fsckDumps() {
    if (
        state.stashedLS.number +
            state.stashedIDB.number +
            state.savedLS.number +
            state.savedIDB.number !==
        0
    ) {
        return;
    }
    ...
    while (
        ...
        try {
            ...
            let [_changedNum, exportedNum, submittedNum, savedNum, _deletedNum] = await archive(
                ...
                true,
                reqresBundledAs,
                reqresUnarchivedIssueAcc,
                reqresUnstashedIssueAcc,
            );
...
async function processRearchiving(getArchivables, reset, andRewrite, andDelete) {
    ...
    await browser.notifications
        .create(`running-rearchive-${rearchiveNum}`, {
            ...
            message: escapeNotification(
                config,
                "Re-archival is now running. The main thread of `Hoardy-Web` will be blocked until completion. This might take awhile.",
            ),
    ...
    if (!allOK) {
        await runNotifier(
            notifyAboutUn,
            "re-archive",
            unrearchivedAccumulator[1].entries(),
            false,
        ).catch(logError);
    }
    ...
    await browser.notifications
        .create("", {
            ...
            message: escapeNotification(
                config,
                `${allOK ? "Re-archived" + (andDelete ? " and deleted" : "") : "Partially re-archived"} ${changedNum} reqres invoking ${actions} and ${deletedNum} deletions from local storage.`,
            ),
...
function syncRearchiveSaved(rrfilter, reset, andRewrite, andDelete) {
    ...
    runSynchronouslyB("rearchiveSaved", async () => {
        ...
        await processRearchiving(
            () => loadSaved(rrfilter, undefined, (v) => [v, null]),
            reset,
            andRewrite,
            andDelete,
        );
    ...
    runSynchronouslyB("deleteSaved", async () => {
        ...
        await browser.notifications
            .create("", {
                ...
                message: escapeNotification(
                    config,
                    `Deleted ${deletedNum} reqres from local storage.`,
                ),
                iconUrl: iconURL(allOK ? "idle" : "error", 128),
                type: "basic",
            })

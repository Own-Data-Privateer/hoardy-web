...
* Parts
- Required:
  - [[./dumb_server/][A dumb archiving server]] (aka =pwebarc-dumb-dump-server.py=) written in [[./dumb_server/pwebarc-dumb-dump-server.py][less than 200 lines of pure Python]] that uses the Python's standard library and nothing else. Have Python installed? You can run it right now, no need to install anything else.
